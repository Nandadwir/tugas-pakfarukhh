...
class Auth extends CI_Controller {
	...
	private function _sendEmail($token, $type)
	{
		...
		$config['smtp_user'] = 'nandadwirahmadan9542@gmail.com'; //isi dengan emailmu
		$config['smtp_pass'] = 'nandadwi87';
		...
		$this->email->from('nandadwirahmadan9542@gmail.com', 'lenathea lipuni');
		$this->email->to($this->input->post('email'));

		if ($type == 'verify') {
			$this->email->subject('Account Verification');
			$this->email->message('Clik this link to verify you account : <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Activated</a>');
		} else if ($type == 'forgot') {
		...
		if ($user) {
			...
			if ($user_token) {
				if (time() - $user_token['data_created'] < (60 * 60 * 24)) {
					...
					'<div class="alert-success" role="alert">' . $email . ' has been Activated!Please login
					</div>');
					redirect('auth');
				} else {
	...
	public function logout()
	{
		...
		$this->session->set_flashdata(
			...
			congratulation,your acount has been created. login!
			</div>'
	...
	public function forgotPassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($user) {
				...
				$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Email is no registered or actived</div>');
				redirect('auth/forgotpassword');
			}
		}
	}
